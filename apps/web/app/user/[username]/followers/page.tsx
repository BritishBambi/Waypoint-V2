// apps/web/app/user/[username]/followers/page.tsx
// Lists every user who follows this profile.
//
// Data strategy (all parallel):
//   1. Profile lookup by username (404 if missing)
//   2. follows WHERE followee_id = profile.id  → follower IDs in recency order
//   3. auth.getUser()                          → current viewer identity
//
// Then a second round:
//   4. profiles WHERE id IN (followerIds)      → display data for each follower
//   5. follows WHERE follower_id = viewer.id   → viewer's own follow set (for buttons)
//
// FollowButton is a client component that handles optimistic follow/unfollow.
...
export default async function FollowersPage({
  ...
  // ── 2. Parallel: follower IDs + current viewer ───────────────────────────
  ...
  const followerIds = (followRows as Array<{ follower_id: string }> ?? []).map(
    (r) => r.follower_id
  );

  // ── 3. Parallel: profile data + viewer's own follows ─────────────────────
  const [followerProfiles, viewerFollowees] = await Promise.all([
  ...
  const profileMap = new Map(
    (followerProfiles as Tables<"profiles">[]).map((p) => [p.id, p])
  );
    ...
    <main className="mx-auto max-w-2xl px-4 py-12">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-zinc-500">
          {orderedFollowers.length}{" "}
          {orderedFollowers.length === 1 ? "follower" : "followers"}
        </p>
      </div>
