...
![Waypoint Logo](assets/images/logo.png)

Waypoint is a social gaming platform built for people who take their games seriously. Inspired by Letterboxd, Waypoint lets users log games, write reviews, rate titles, track playtime via Steam, follow friends, build curated lists, and discover new games through their community. It's the platform for gamers who want their library to mean something.

[View Live Site](https://waypoint-v2-web.vercel.app)
...
Waypoint started as a Code Institute portfolio project — a Django-based gaming log site. This version is a ground-up rebuild using a modern full-stack architecture. The goal was simple: build the platform that actually deserves to exist for gamers. Not another review aggregator, but a social layer for the people who care deeply about what they play.

The current version is a live, deployed web application with real users, Steam integration that automatically syncs your gaming library, and a growing feature set that keeps getting better.
...
Gamers have no good home. Letterboxd exists for film fans — a beautiful, social, opinionated platform where taste matters. Nothing like that exists for games. Goodreads exists for book readers. Games have... spreadsheets and Discord servers.

Waypoint is the answer to that gap.

#### Ideal User

- Someone with a large gaming library who wants to track what they've played
- A gamer who wants to share opinions and read reviews from real people, not critics
- Someone who wants to follow friends and see what they're playing right now
- A completionist who wants to track 100% achievement runs and show them off
- Anyone who's ever thought "I should really keep track of all the games I've played"

#### Goals

- Make logging a game as frictionless as possible
- Surface social activity naturally — what your friends are playing matters
...
- **User Story — View Game Detail**: As a user, I want to view detailed information about any game so that I know what I'm logging.
...
- **User Story — Equip a Title**: As a user, I want to choose which title displays on my profile so that I can show off the one I'm proudest of.
...
Profiles are built around the library. Favourite games displayed prominently, a showcase section for a pinned review or lists, and a library carousel below. Stats accessible from a prominent link.
...
Waypoint uses a dark-first design language. Deep backgrounds (near-black), muted surface layers, and a brand purple accent (`#7c3aed`) that ties the UI together. Status colours and title colours are the exception — these use intentionally vivid hues to draw the eye and reward discovery.
...
Waypoint uses Supabase (PostgreSQL) with Row Level Security enabled on all tables. The schema has grown significantly since the initial build and now covers the full feature set.
...
The logged-in homepage is the social hub of Waypoint. It's designed to answer the most important question a gamer has when they open the app: *what are my friends playing?*
...
A live grid of recent activity from followed users. Shows Playing, Completed, and Dropped entries — never Wishlist or Backlog, keeping the feed meaningful.

**Popular Right Now**

A horizontal carousel of trending games sourced from IGDB's popularity data. A quick pulse on what's hot in the gaming world right now.

**Coming Soon**

Upcoming games with release date badges so users can keep an eye on what's around the corner.

**Who to Follow**

Three user suggestions powered by taste-matching. Users who share the most games in common with you appear first. Popular users fill any remaining slots. Follow button fades the card out on click.

**Recent and Trending Lists**

Side by side: the two most recently created public lists, and the two lists that have earned the most likes in the past seven days. Gives list creators a meaningful reason to make good content.
...
- See the 100% completion badge if they've cleared every achievement
...
![Game Detail Screenshot](assets/screenshots/game-detail.png)

#### Achievement Modal

For users with Steam connected, the achievement section sits below the progress bar on game detail pages. A scrollable row of achievement icons — colour for unlocked, greyscale for locked — gives an instant visual summary. Clicking anywhere on the section opens the full achievement modal.
...
![Achievement Modal Screenshot](assets/screenshots/achievements.png)

#### 100% Completion Badge

A gold trophy badge appears below the cover art for any game where the logged-in user has achieved 100% Steam achievements. A hover tooltip confirms "100% Achievements Unlocked". Future updates will replace the placeholder icon with custom badge artwork.
...
From the edit profile page, click **Connect Steam**. You'll be redirected to Steam's OpenID login, then returned to Waypoint with your Steam profile linked. Your Steam avatar and display name are stored and visible on your Waypoint profile via a badge with a direct link to your Steam page.

#### Syncing Your Library

The **Sync Steam** button runs a full library sync in the background. Here's what happens:
...
5. Any games you've 100% that have a Waypoint title attached are automatically awarded

The first sync is the big one — it populates Waypoint's game database from your library, meaning every game you own becomes part of the platform for all users.

![Steam Sync Screenshot](assets/screenshots/steam-sync.png)

---

### Library

The library page is the complete grid view of everything a user has logged — Playing, Completed, Backlog, and Dropped. Wishlist lives separately.

**Status Tabs**

Tabs across the top filter the grid: All, Playing, Completed, Dropped, Backlog. Each tab shows the count for that status. Filtering is instant, client-side — no page reload.
...
- Note indicator (bottom left) if a private note exists and no review — hover shows a preview of the note
...
A dedicated page for games the user wants to play but doesn't own yet. Displayed as a cover grid. Unreleased games show a release date badge so users know when to expect them. Wishlist entries never appear in the activity feed — they're personal.

---

### Activity Feed

The full activity feed at `/activity` shows all recent Playing, Completed, and Dropped entries from followed users, paginated at 30 per page. Clicking any entry goes to the game detail page. Following a user means caring about what they actually play — Backlog and Wishlist entries are intentionally excluded to keep the signal strong.
...
Scrollable previews of the user's library and wishlist. Links to the full pages.
...
Stats are public — visitors can see any user's breakdown.
...
Lists can be liked by other users and appear in the **Trending Lists** section on the homepage when they pick up engagement. Recently created lists appear in **Recent Lists**.

---

### Reviews and Reactions

Reviews live on game detail pages and a user's profile. Each review has a 1–10 rating and an optional written body. Spoiler protection is available — spoiler reviews are blurred until the reader clicks to reveal, unless they've already played the game themselves.
...
The title system is one of Waypoint's most distinctive features. Titles are curated rewards for genuine achievement — specifically, 100% Steam achievement completion on selected games.

Titles are not earned by spending time on the platform or clicking things. They're earned by actually completing a game to its fullest.

**How it works:**

When a Steam sync detects 100% achievement completion on a game that has a Waypoint title attached, the title is automatically awarded. The user receives a toast notification immediately and a persistent notification in their notification centre.

**Equipped Title Display:**

Users can choose which of their earned titles to display from a dropdown on the edit profile page. The active title appears below the username on the profile page — a circular game icon (cropped to circle) followed by the title name in the game's signature colour.
...
![Title Display Screenshot](assets/screenshots/titles.png)
...
- **Title Unlocked** — you've earned a new title, showing the game cover and title name in its signature colour
...
All users on the platform listed below the game search, ranked by likes received then follower count. A quick way to find active community members.
...
The current title icons use circular-cropped Steam app icons. The plan is to commission custom badge artwork from [Julie Ucha](https://www.julieucha.com) for each title — unique, illustrated badges that make earning a title feel like a real moment.
...
The stats page will gain a platform breakdown section — how many of your games are on PC, PlayStation, Xbox, Nintendo Switch, and so on.
...
A dedicated browse page for discovering games by genre, decade, platform, or popularity — beyond the homepage carousels.
...
> **Important**: After every deployment of `steam-sync`, go to Supabase Dashboard → Edge Functions → steam-sync → Details and turn **OFF** "Verify JWT with legacy secret". Supabase resets this to ON on every deploy and it will cause all sync requests to fail with a 401.
...
**Verify JWT with legacy secret reset** — Supabase resets the "Verify JWT with legacy secret" setting to ON after every Edge Function deployment. This caused all steam-sync requests to fail with 401. Documented as a known deployment step — must be manually turned OFF after every deploy.
...
- **Brad's notes save issue** — One user reported notes not saving on backlog entries. The notes field is correctly shown for backlog status in code. Suspected cause is a stale session token — pending confirmation after a fresh login test.
...
- [IGDB](https://www.igdb.com) — The game database that makes this platform possible. Game data, cover art, screenshots, and release information all come from IGDB via Twitch's API.
...
- [Letterboxd](https://letterboxd.com) — The gold standard for social film logging. The spiritual blueprint for what Waypoint aims to be for games.
- [RAWG.io](https://rawg.io) — Early inspiration for game data presentation.

### Tools

- [Claude Code](https://claude.ai/code) — AI-assisted implementation in VS Code.
...
The original Waypoint was built as a Code Institute Portfolio Project 4 using Django. That project proved the concept and is what led to this full rebuild. You can view it [here](https://github.com/BritishBambi/The-Waypoint-PP4).
